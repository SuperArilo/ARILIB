...
public class TeleportingServiceImpl implements TeleportingService {
    ...
    @Override
    public TeleportingService teleport(Entity entity, Location targetLocation) {
        ...
        Lib.instance.getScheduler().runAtRegion(targetLocation, i -> {
            ...
            Lib.instance.getScheduler().runAtEntity(entity, t -> {
                boolean isFolia = Scheduler.isFolia();
                boolean cancelled = false;

                if (isFolia) {
                    PlayerTeleportEvent event = new PlayerTeleportEvent(player, entity.getLocation(), targetLocation, PlayerTeleportEvent.TeleportCause.PLUGIN);
                ...
                entity.teleportAsync(targetLocation, PlayerTeleportEvent.TeleportCause.PLUGIN).thenAccept(status -> {
                    if (status) {
                        Lib.instance.getScheduler().runAtEntity(player, o -> player.playSound(Sound.sound(ENTITY_ENDER_EYE_DEATH, SoundCategory.PLAYERS, 1.0f, 1.0f)), null);
                    }
                    this.after.run();
